...
src_compile() {
	COMP_FLAGS=CC="\"$(tc-getCC)\" --prefix=${ED}/usr --actual-prefix=${EPREFIX}/usr --libdir=${EPREFIX}/usr/$(get_libdir)"
	if ! use nls; then
...
src_install(){
	escons install LIBDIR=${EPREFIX}/usr/$(get_libdir) --prefix="${ED}"/usr
