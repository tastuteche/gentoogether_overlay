...
_update_depmod() {
	...
	if [ -r "${KV_OUT_DIR}"/System.map ]; then
		depmod -ae -F "${KV_OUT_DIR}"/System.map -b "${EROOT}" -r "${1}"
		eend $?
	else
		ewarn
...
_firmwares_src_install() {
	...
	cd "${S}" || die
	emake INSTALL_FW_PATH="${ED}/lib/firmware" firmware_install || die "cannot install firmwares"
}

_kernel_sources_src_install() {
	_kernel_copy_config ".config"
	kernel-2_src_install
	cd "${ED}${KV_OUT_DIR}" || die
	local oldarch="${ARCH}"
	unset ARCH
...
_kernel_src_install() {
	...
	for ver_dir in "${version_h_dir}" "${version_h_dir2}"; do
		version_h="${EROOT}${KV_OUT_DIR/\//}/${ver_dir}/version.h"
		if [ -f "${version_h}" ]; then
	...
	cp -Rp "${WORKDIR}"/lib/* "${ED}/" || die "cannot copy /lib over"
	...
	cd "${ED}"/lib/modules/* || die "cannot enter /lib/modules directory, more than one element?"
	# cleanup previous
	...
	# drop ${ED}/lib/firmware, virtual/linux-firmwares provides it
	...
	if [ -n "${K_WORKAROUND_SOURCES_COLLISION}" ]; then
		...
		if [ -f "${EROOT}/${make_file}" ]; then
			elog "Removing ${ED}/${make_file}"
			rm -f "${ED}/${make_file}"
		fi
	fi
...
sabayon-kernel_grub2_mkconfig() {
	if [ -x "${EROOT}usr/sbin/grub2-mkconfig" ]; then
		# Grub 2.00
		"${EROOT}usr/sbin/grub2-mkconfig" -o "${EROOT}boot/grub/grub.cfg"
	elif [ -x "${EROOT}sbin/grub-mkconfig" ]; then
		# Grub 1.99
		"${EROOT}sbin/grub-mkdevicemap" --device-map="${EROOT}boot/grub/device.map"
		"${EROOT}sbin/grub-mkconfig" -o "${EROOT}boot/grub/grub.cfg"
	else
		echo
...
_get_real_extraversion() {
	make_file="${EROOT}${KV_OUT_DIR}/Makefile"
...
_dracut_initramfs_create() {
  ...
  local karch="${2}"
	...
	if [ "${K_DRACUT_LIVECD}" = "1" ]; then
		...
		"${EROOT}boot/initramfs-genkernel-${karch}-${kver}"
	else
...
sabayon-kernel_pkg_postinst() {
	if _is_kernel_binary; then
		...
		if use amd64 || use x86; then
			...
			# grub-legacy
			if [ -x "${EROOT}usr/sbin/grub-handler" ]; then
				"${EROOT}usr/sbin/grub-handler" add \
...
sabayon-kernel_pkg_postrm() {
	if _is_kernel_binary; then
		...
		if use amd64 || use x86; then
			...
			if [ -x "${EROOT}usr/sbin/grub-handler" ]; then
				"${EROOT}usr/sbin/grub-handler" remove \
					"/boot/kernel-genkernel-${kern_arch}-${KV_FULL}" \
					"/boot/initramfs-genkernel-${kern_arch}-${KV_FULL}"
			fi
